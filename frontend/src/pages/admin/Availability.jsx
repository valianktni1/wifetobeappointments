...
export default function Availability() {
  ...
  const saveHours = async () => {
    try { await api.put(`/shops/${shopId}/availability`, { hours, slot_step: step }); toast.success("Opening hours saved"); }
    catch (e) { toast.error(apiErr(e)); }
  };

  const addBlock = async () => {
    if (!newBlock.date) { toast.error("Choose a date to block"); return; }
    try { await api.post(`/shops/${shopId}/blocked-dates`, newBlock); setNewBlock({ date: "", reason: "" }); load(); toast.success("Date blocked"); }
    catch (e) { toast.error(apiErr(e)); }
  };
          ...
          <div className="space-y-2" data-testid="blocked-list">
            ...
            {blocked.map((b) => (
              <div key={b.id} className="flex items-center justify-between border-b pb-2" style={{ borderColor: "var(--line)" }}>
                <span className="font-sans-j text-sm">{format(new Date(b.date), "EEE d MMM yyyy")}{b.reason ? ` — ${b.reason}` : ""}</span>
